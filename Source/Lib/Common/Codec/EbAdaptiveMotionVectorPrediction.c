...
static INLINE int is_global_mv_block(const MbModeInfo *const mbmi,
    ...
    if (rf[1] == NONE_FRAME) {
        ...
        for (ref = 0; ref < 2; ++ref) {
            if (candidate->ref_frame[ref] == rf[0]) {
                ...
                if (is_global_mv_block(&candidate_mi->mbmi, gm_params[rf[0]].wmtype))
        ...
        if (candidate->ref_frame[0] == rf[0] && candidate->ref_frame[1] == rf[1]) {
            ...
            for (ref = 0; ref < 2; ++ref) {
#if USE_CUR_GM_REFMV
                if (is_global_mv_block(&candidate_mi->mbmi, gm_params[rf[ref]].wmtype))
